...
#############################
### author: jm - oct2023
### Only the classes to remenber it
...
class MSSQLdb:
    ...
    # to count the number of objects created from MSSQLdb
    __cnt = 0
    # to register DB connections results
...
#-NEED to create intrinsic subdirs (of course 'listas/' exits if not impossible to read .xslx)
...
for i in range(len(mini_dfs)):  # 1st loop, for e/mini_df - Cnx Srv-DB
    try:                        # 1. connect to de DB - db_obj
        ...
        wrt_info(ln)
        db = MSSQLdb(srv, dbnm, 'user1', 'pass1')
    except Exception as e:
